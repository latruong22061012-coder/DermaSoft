...
class LichHen extends Model
{
    ...
    /**
     * Lấy lịch hẹn chưa được xác nhận (TrangThai = 1)
     */
    public static function getPendingAppointments(): array
    {
        $sql = "SELECT * FROM LichHen WHERE TrangThai = 1 ORDER BY ThoiGianHen ASC";
    ...
    public static function createAppointment(array $data): int|false
    {
        return self::create([
            ...
            'TrangThai' => 1,  // Mặc định là pending
            'GhiChu' => $data['GhiChu'] ?? ''
        ]);
    }
    ...
    /**
     * Xác nhận lịch hẹn (TrangThai = 2) - Windows App gọi
     */
    public static function confirm(int $id): int
    {
        return self::updateStatus($id, 2);
    }
}
